...
def perform_data_validation(
    ...
    check_for_nas: bool = True,
    check_for_ranges: bool = True,
    ...
    Arguments:
        ...
        columns_to_ignore - columns to ignore during validationg. useful if backend sends a value to the
            endpoint that is not needed in the model
    Returns:
        df - pandas df with errors column appended, describing data validation errors
    """

    df_columns_to_check = [c for c in df.columns if c not in columns_to_ignore]
    ...
    for i, row in df.iterrows():
        ...
        if check_for_ranges:
            ...
            for col in intersection(df_columns_to_check, feature_boundaries.keys()):
...
def format_numeric_data_types(df: pd.DataFrame) -> pd.DataFrame:
    """
    format df columns to be numeric datatypes

    Parameters
    ----------
    df - input dataframe with columns to be formatted

    Returns
    -------
    formatted_df - dataframe with columns formatted as numeric types

    """
    non_numeric_columns = ["time"]
    cols_to_format = [c for c in df if c not in non_numeric_columns]
...
def unpack_dataframes(
    ...
    for df_metadata, df_as_numpy in zip(dfs_metadata, dfs_as_numpy):
        ...
        df = format_numeric_data_types(df)
